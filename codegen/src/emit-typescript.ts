...
// @ts-nocheck
import { getOCCTModule } from '../../manual/ts/module-registry.js';
`;
...
function emitEnum(enumName: string, enumDef: EnumDef): string {
  ...
  for (const value of enumDef.values) {
    lines.push(`  get ${value}(): number { return getOCCTModule().${enumName}.${value}.value; },`);
  }
  lines.push(`};`);
...
function emitConstructorBody(
  ...
  const argExprs = ctor.args.map((arg, i) => {
    ...
    if (isOcctEnum(arg.type)) {
      return `getOCCTModule().${arg.type}.values[args[${i}]]`;
    }
    return `args[${i}]`;
  });
  ...
  if (ctor.factory) {
    ...
    lines.push(`${indent}const Module = getOCCTModule();`);
...
function emitSimpleMethodBody(
  ...
  const argExprs = inputArgs.map(arg =>
    (isRegisteredClass(arg.type) && !isOcctEnum(arg.type)) ? `${arg.name}._handle` :
    isOcctEnum(arg.type) ? `getOCCTModule().${arg.type}.values[${arg.name}]` : arg.name
  );
  const argsStr = argExprs.join(', ');
